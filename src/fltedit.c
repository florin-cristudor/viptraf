...
int loadfilter(const char *filename, struct filterlist *fl, int resolve)
{
    ...
    do
    {
        ...
        if (br > 0)
        {
            ...
            if (resolve)
            {
                if (!nametoaddr(fe->hp.s_fqdn, &fe->src))
                {
                    free(fe);
                    continue;
                }

                if (!nametoaddr(fe->hp.d_fqdn, &fe->dst))
                {
                    free(fe);
                    continue;
                }
                if (fe->src.type == 4) fe->src.un.ipv4.mask = inet_addr(fe->hp.s_mask);
                if (fe->dst.type == 4) fe->dst.un.ipv4.mask = inet_addr(fe->hp.d_mask);
                if (fe->src.type == 6) fe->src.un.ipv6.prefix = (unsigned long) atoi(fe->hp.s_mask);
                if (fe->dst.type == 6) fe->dst.un.ipv6.prefix = (unsigned long) atoi(fe->hp.d_mask);
            }
			if (fl->head == NULL) {
				fl->head = fe;
				fe->prev_entry = NULL;
