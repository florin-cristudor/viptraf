...
void print_udp(char * buffer, int size, struct othptabent *entry)
{
    if (!buffer) return;
    snprintf(buffer, size, "UDP   (%4u/%6s, %5s->%5s) from %15s to %15s", entry->pkt_length, entry->iface,
        entry->un.udp.s_sname, entry->un.udp.d_sname, entry->s_fqdn, entry->d_fqdn
    );
}
...
void print_icmp(char * buffer, int size, struct othptabent *entry)
{
    ...
    {
        case ICMP_ECHOREPLY:
            pDescription = "echo rply ";
            break;
        case ICMP_ECHO:
            pDescription = "echo req  ";
            break;
        case ICMP_DEST_UNREACH:
            pDescription = "dest unrch";
            switch (entry->un.icmp.code)
            {
                ...
                case ICMP_FRAG_NEEDED:
                    ...
                    break;
                case ICMP_SR_FAILED:
                    pAdditional = "src rte fail";
                    break;
                case ICMP_NET_UNKNOWN:
                    pAdditional = "net unkn";
                    break;
                case ICMP_HOST_UNKNOWN:
                    pAdditional = "host unkn";
                    break;
                case ICMP_HOST_ISOLATED:
                    pAdditional = "src isltd";
                    break;
                case ICMP_NET_ANO:
                    pAdditional = "net comm denied";
                    break;
                case ICMP_HOST_ANO:
                    pAdditional = "host comm denied";
                    break;
                case ICMP_NET_UNR_TOS:
                    pAdditional = "net unrch for TOS";
                    break;
                case ICMP_HOST_UNR_TOS:
                    pAdditional = "host unrch for TOS";
                    break;
                case ICMP_PKT_FILTERED:
                    pAdditional = "pkt fltrd";
                    break;
                case ICMP_PREC_VIOLATION:
                    pAdditional = "prec violtn";
                    break;
                case ICMP_PREC_CUTOFF:
                    pAdditional = "prec cutoff";
                    break;
                default:
                    pAdditional = "unknown";
                    break;
        ...
        case ICMP_SOURCE_QUENCH:
            pDescription = "src qnch  ";
            break;
        case ICMP_REDIRECT:
            pDescription = "redirct   ";
            break;
        case ICMP_TIME_EXCEEDED:
            pDescription = "time excd ";
            break;
        case ICMP_PARAMETERPROB:
            pDescription = "param prob";
            break;
        case ICMP_TIMESTAMP:
            pDescription = "timestmp req";
            break;
        case ICMP_INFO_REQUEST:
            pDescription = "info req  ";
            break;
        case ICMP_INFO_REPLY:
            pDescription = "info rep  ";
            break;
        case ICMP_ADDRESS:
            pDescription = "addr mask req";
            break;
        case ICMP_ADDRESSREPLY:
            pDescription = "addr mask rep";
            break;
        default:
            pDescription = "bad/unkn  ";
            break;
    }

    snprintf(buffer, size, "ICMP  (%4u/%6s, %s, %s) from %15s to %15s", entry->pkt_length, entry->iface,
        pDescription, pAdditional,  entry->s_fqdn, entry->d_fqdn);
}
...
void print_icmpv6(char * buffer, int size, struct othptabent *entry)
{
    ...
    switch (entry->un.icmp6.type)
    {
        case ICMP6_DST_UNREACH:
            ...
            switch (entry->un.icmp6.code)
            {
                ...
                case ICMP6_DST_UNREACH_NOTNEIGHBOR:
                    pAdditional = "not neigh";
#else
                case ICMP6_DST_UNREACH_BEYONDSCOPE:
                    pAdditional = "not beyondsp";
#endif
                    break;
                case ICMP6_DST_UNREACH_ADDR:
                    pAdditional = "unreach addr";
                    break;
        ...
        case ICMP6_PACKET_TOO_BIG:
            pDescription = "pkt too big";
            break;
        case ICMP6_TIME_EXCEEDED:
            pDescription = "time exceeded";
            break;
        ...
        case ND_ROUTER_SOLICIT:
            pDescription = "router sol";
            break;
        case ND_ROUTER_ADVERT:
            pDescription = "router adv";
            break;
#ifdef ICMP6_MEMBERSHIP_QUERY
        case ICMP6_MEMBERSHIP_QUERY:
            pDescription = "mbrship query";
            break;
#endif
#ifdef ICMP6_MEMBERSHIP_REPORT
        case ICMP6_MEMBERSHIP_REPORT:
            pDescription = "mbrship report";
            break;
#endif
#ifdef ICMP6_MEMBERSHIP_REDUCTION
        case ICMP6_MEMBERSHIP_REDUCTION:
            pDescription = "mbrship reduc";
            break;
#endif
        case ND_NEIGHBOR_SOLICIT:
            pDescription = "neigh sol";
            break;
        case ND_NEIGHBOR_ADVERT:
            pDescription = "neigh adv";
            break;
        case ND_REDIRECT:
            pDescription = "redirect";
            break;
        default:
            pDescription = "bad/unkn";
            break;
    ...
    snprintf(buffer, size, "ICMPv6(%4u/%6s, %15s -> %15s) %s,  %s", entry->pkt_length, entry->iface,
        entry->s_fqdn, entry->d_fqdn, pDescription, pAdditional);
}
            ...
            unsigned int target_row, int logging, FILE * logfile)
{
    ...
    switch (entry->protocol)
    {
        case IPPROTO_UDP:
            ...
            print_udp(msgstring, sizeof(msgstring), entry);
            message_complete=1;
            break;
        case IPPROTO_ICMP:
            wattrset(table->othpwin, STDATTR);
            print_icmp(msgstring, sizeof(msgstring), entry);
            message_complete=1;
            break;
        case IPPROTO_ICMPV6:
            wattrset(table->othpwin, ICMPV6ATTR);
            print_icmpv6(msgstring, sizeof(msgstring), entry);
            message_complete=1;
            break;
    ...
    if (!(entry->fragment))
    {
        ...
        if (entry->protocol == IPPROTO_OSPFIGP) {
    ...
    if (!message_complete)
    {
        ...
        sprintf(scratchpad, "(%3u bytes) ", entry->pkt_length);
        strcat(msgstring, scratchpad);

        if ((entry->protocol == IPPROTO_UDP) && (!(entry->fragment)))
        {
            sprintf(scratchpad, "from %.70s:%s to %.70s:%s", entry->s_fqdn,
