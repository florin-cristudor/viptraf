...
static int addr_in_net(unsigned long addr, struct filter_addr *fa)
{
    if (NULL == fa)
        return 0;
    if (fa->type != 4)
        return 0;
    return (addr & fa->un.ipv4.mask) == (fa->un.ipv4.addr.s_addr & fa->un.ipv4.mask);
}


unsigned char byte_mask[9] = { 0x00, 0x80, 0xC0, 0xE0, 0xF0, 0xF8, 0xFC, 0xFE, 0xFF };

static int addr6_in_net(struct in6_addr *addr, struct filter_addr *fa)
{
    if (NULL == addr || NULL == fa) return 0;
    if (fa->type != 6)
    {
        return 0;
    }
    int next_0 = 0;
    for(int i=0, prefix=fa->un.ipv6.prefix; i<16; i++)
    {
        if ( prefix < 8 ) next_0 = 1;

        unsigned char mask;
        if (prefix >= 8)
            mask = byte_mask[8];
        else
            mask = byte_mask[prefix];
        if ((addr->s6_addr[i] & mask) != (fa->un.ipv6.addr.s6_addr[i] & mask) )
            return 0;
        if (next_0)
            prefix = 0;
        else
...
int ip6filter(struct in6_addr *saddr, struct in6_addr *daddr, in_port_t sport, in_port_t dport, unsigned int protocol)
{
    struct filterent *fe;
    int accept_pkt = 0, match;

    if(ofilter.fl.head == NULL) accept_pkt = 1;
    ...
    for (fe = ofilter.fl.head; fe != NULL; fe = fe->next_entry)
    {
        ...
        switch(protocol)
        {
            case IPPROTO_TCP:
            case IPPROTO_UDP:
                if (!fe->src.is_null) match &= addr6_in_net(saddr, &fe->src);
                if (!fe->dst.is_null) match &= addr6_in_net(daddr, &fe->dst);
                if (fe->hp.sport1) match &= port_in_range(sport, fe->hp.sport1, fe->hp.sport2);
                if (fe->hp.dport1) match &= port_in_range(dport, fe->hp.dport1, fe->hp.dport2);
                if (protocol == IPPROTO_UDP) match &= fe->hp.filters[F_UDP];
                if (protocol == IPPROTO_TCP) match &= fe->hp.filters[F_TCP];
                break;
            case IPPROTO_ICMPV6:
                //icmp
                if (!fe->src.is_null) match &= addr6_in_net(saddr, &fe->src);
                if (!fe->dst.is_null) match &= addr6_in_net(daddr, &fe->dst);
                match &= fe->hp.filters[F_IPV6_ICMP];
                break;

            default:
                return 0;
        }
        if (toupper(fe->hp.match_opposite == 'Y'))
        {
            if(match) match = 0;
            else match = 1;
        }
        if(!match) continue; //maybe the reset have luck
        //we are in match
        if(toupper(fe->hp.reverse) != 'E')
        {
             debug_log("%s: ip6filter proto %i passed", __FUNCTION__, protocol);
            return 1; //we have to include this
        }
        //drop the packet
        return 0;
    }
...
//    int fltexpr2;
    ...
    for (fe = ofilter.fl.head; fe != NULL; fe = fe->next_entry)
    {
        ...
        switch(protocol)
        {
            case IPPROTO_TCP:
            case IPPROTO_UDP:
                if (fe->src.un.ipv4.addr.s_addr) match &= addr_in_net(saddr, &fe->src);
                if (fe->dst.un.ipv4.addr.s_addr) match &= addr_in_net(daddr, &fe->dst);
                if (fe->hp.sport1) match &= port_in_range(sport, fe->hp.sport1, fe->hp.sport2);
                if (fe->hp.dport1) match &= port_in_range(dport, fe->hp.dport1, fe->hp.dport2);
                if (protocol == IPPROTO_UDP) match &= (fe->hp.filters[F_UDP] || fe->hp.filters[F_ALL_IP]);
                if (protocol == IPPROTO_TCP) match &= (fe->hp.filters[F_TCP] || fe->hp.filters[F_ALL_IP]);
                break;
            ...
            case IPPROTO_ICMP:
                ...
                if (fe->src.un.ipv4.addr.s_addr) match &= addr_in_net(saddr, &fe->src);
                if (fe->dst.un.ipv4.addr.s_addr) match &= addr_in_net(daddr, &fe->dst);
                match &= (fe->hp.filters[F_ICMP] || fe->hp.filters[F_ALL_IP]);
                break;

            default:
                return 0;
/*                fltexpr1 = addr_in_net(saddr, fe->saddr, fe->smask) && addr_in_net(daddr, fe->daddr, fe->dmask);
                if (fe->hp.match_opposite == 'Y')
                {
                    fltexpr2 = addr_in_net(saddr, fe->daddr, fe->dmask) && addr_in_net(daddr, fe->saddr, fe->smask);
                }
                else
                    fltexpr2 = 0;
        ...
        if(!match) continue; //maybe the reset have luck
        //we are in match
        if(toupper(fe->hp.reverse) != 'E') return 1; //we have to include this
        //drop the packet
        return 0;
    }
    return accept_pkt;
