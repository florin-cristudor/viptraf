         ...
         struct sockaddr_storage *daddr)
{
    ...
    debug_log("%s: dupa while is null %i", __FUNCTION__, hashptr == NULL);
    ...
    {  /* needed to avoid SIGSEGV */
        ...
        {
        debug_log("%s: return null", __FUNCTION__);

            return NULL;
        }
        else
        {
    debug_log("%s: dupa while return some", __FUNCTION__);
    ...
    {
    debug_log("%s: dupa while return null final", __FUNCTION__);
...
void flushclosedentries(struct tcptable *table)
{
    ...
    if(!table->firstvisible || !table->firstvisible->index) return;
